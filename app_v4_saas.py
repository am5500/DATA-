...
def generate_insights(df: pd.DataFrame, schema: dict, primary: str | None) -> list[dict]:
    ...
    if primary and cat_cols:
        ...
        insights.append({"icon":"🏆","title":"Top Performer",
                          "body":f"<b>{top_name}</b> leads {cat_cols[0]} with {pct:.1f}% of total.",
                          "highlight": fmt_number(top_val)})

        # Bottom performer
        bot_name = grp.idxmin()
        bot_val  = grp.min()
        insights.append({"icon":"⚠️","title":"Needs Attention",
                          "body":f"<b>{bot_name}</b> has the lowest {primary}.",
                          "highlight": fmt_number(bot_val)})

        # Best region / second cat
        if len(cat_cols) > 1:
            grp2 = df.groupby(cat_cols[1], observed=True)[primary].sum()
            best2 = grp2.idxmax()
            insights.append({"icon":"🌍","title":f"Best {cat_cols[1]}",
                              "body":f"<b>{best2}</b> ranks #1 in {cat_cols[1]}.",
                              "highlight": fmt_number(grp2.max())})

    if primary:
        # Largest single value
        max_val = df[primary].max()
        max_row = df.loc[df[primary].idxmax()]
        cat_hint = f" ({max_row[cat_cols[0]]})" if cat_cols else ""
        insights.append({"icon":"💎","title":f"Largest {primary}",
                          "body":f"Single largest value in the dataset{cat_hint}.",
                          "highlight": fmt_number(max_val)})
    ...
    if primary and schema["date"]:
        ...
        if len(monthly) >= 2:
            ...
            insights.append({"icon":"📈","title":"Peak Month",
                              "body":f"Highest {primary} in <b>{peak_m.strftime('%B %Y')}</b>.",
            ...
            if len(last2) == 2:
                ...
                insights.append({"icon":"📊","title":"Recent Trend",
                                  "body":f"Last month vs previous month.",
                                  "highlight": f'<span style="color:{color}">{arrow} {abs(chg):.1f}%</span>'})
    ...
    if primary:
        ...
        if outlier_count > 0:
            insights.append({"icon":"🔍","title":"Outliers Detected",
                              "body":f"Records in {primary} outside 3 standard deviations.",
                              "highlight": f"{outlier_count:,} rows"})
    ...
    if len(num_cols) >= 2:
        ...
        if c1 != c2:
            ...
            insights.append({"icon":"🔗","title":"Strongest Correlation",
                              "body":f"{direction.title()} link between <b>{c1}</b> and <b>{c2}</b>.",
                              "highlight": f"r = {r:.3f}"})
...
def main_enhanced():
    ...
    # Compute real trend percentages from date column if available
    def compute_trend(df, schema, primary, kpi_label):
        """Return (pct_change, is_positive) or (None, None) if not computable."""
    ...
    # Pre-compute one global trend
    trend_pct, trend_pos = compute_trend(df, schema, primary, "")

    kpi_html = '<div class="kpi-grid">'
    for i, kpi in enumerate(kpis):
        icon = KPI_ICONS[i % len(KPI_ICONS)]
        # Use real trend for primary KPI, offset variations for others
...
# ══════════════════════════════════════════════════════════════════════════════
# ██  AI ANALYST + EXECUTIVE SUMMARY + SMART INSIGHTS  ████████████████████████
# ══════════════════════════════════════════════════════════════════════════════

# ─────────────────────────────────────────────
# AI ANALYST STYLES (appended once)
# ─────────────────────────────────────────────
AI_ANALYST_CSS = """
<style>
/* ── AI Analyst tab ── */
...
# ─────────────────────────────────────────────
# GENERATE INSIGHTS (local, stats-based)
# ─────────────────────────────────────────────
def generate_insights(df: pd.DataFrame, schema: dict, primary: str | None) -> list[dict]:
...
def call_openai(messages: list[dict], api_key: str) -> str:
    """Call OpenAI chat completions API with streaming (collected)."""
...
def call_gemini(messages: list[dict], api_key: str) -> str:
    ...
    # Remove system messages (Gemini doesn't support them the same way)
...
def render_ai_analyst_tab(df: pd.DataFrame, schema: dict, primary: str | None, health: dict):
    ...
    provider_short = "openai" if "OpenAI" in st.session_state.get("ai_provider_radio", "OpenAI") else "gemini"
    ...
    if not st.session_state.ai_chat_history:
        ...
        for msg in st.session_state.ai_chat_history:
            ...
            if msg["role"] == "user":
                ...
                # Convert markdown-style bold/bullets to HTML
...
def _expiry_date(plan: str) -> str:
    days = 30 if plan == "free" else 36500  # ~100 years = permanent for pro
...
def _dashboard_url(dash_id: str) -> str:
    try:
        base = st.context.url if hasattr(st, "context") and hasattr(st.context, "url") else "http://localhost:8501"
    except Exception:
        base = "http://localhost:8501"
    return f"{base}?dashboard={dash_id}"
...
def save_dashboard(file_name: str, rows: int, cols: int, template: str) -> dict:
    ...
    # Free plan: max 1 dashboard
    if plan == "free" and len(dashboards) >= 1:
        # Overwrite the single existing one
...
def gate_pro(feature_name: str) -> bool:
    """Return True if allowed. If not pro, show upgrade message."""
    if is_pro():
        return True
    st.markdown(f"""
    <div class='upgrade-banner'>
        <div class='upgrade-banner-text'>
            🔒 <b>{feature_name}</b> is a <b>Pro</b> feature.
            Upgrade to unlock unlimited dashboards, AI chat, exports, and more.
        </div>
...
def render_share_panel(record: dict):
    ...
    # Copy link button (Streamlit workaround — show URL in text input for easy copy)
...
# ──────────────────────────────────────────────────────────────────────────────
# PATCH main_enhanced — wire everything into tabs
# ──────────────────────────────────────────────────────────────────────────────
_original_main_enhanced = main_enhanced   # keep reference before patching

def main_enhanced():           # noqa: F811  (intentional override)
    """
    Patched main_enhanced:
    - Wraps the full dashboard in two tabs: 📊 Dashboard | 🤖 AI Analyst
    - Injects Executive Summary and Smart Insights into the Dashboard tab
    """
    init_session_state()
    inject_theme()
    st.markdown(EXTRA_CSS, unsafe_allow_html=True)
