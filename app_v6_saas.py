"""
Dynamic Dashboard Generator with AI Chart Suggestions (Groq)
Uses Groq API (Llama 3) to understand column meanings and suggest optimal charts.
"""
...
# ─────────────────────────────────────────────
# GROQ API KEY (hardcoded, hidden from UI)
# ─────────────────────────────────────────────
GROQ_API_KEY = "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"
...
# ─────────────────────────────────────────────
# GLOBAL STYLES (full CSS as in original)
# ─────────────────────────────────────────────
st.markdown("""
<style>
/* ── Google Fonts ── */
...
def build_kpis(df: pd.DataFrame, schema: dict, primary: str | None) -> list[dict]:
    ...
    for col in schema["categorical"][:3]:
        if len(kpis) >= 8:
            break
        kpis.append({"label":f"Unique {col}","value":fmt_number(df[col].nunique()),"sub":"distinct values","color":colors[len(kpis)%len(colors)]})
    if schema["date"]:
        dcol = schema["date"][0]
...
# ─────────────────────────────────────────────
# CHART FUNCTIONS (unchanged from original, all present)
# ─────────────────────────────────────────────
...
def init_session_state():
    defaults = {
        ...
        "openai_api_key": "",
        "gemini_api_key": "",
        "groq_api_key": GROQ_API_KEY,  # pre-filled
        "ai_charts": None,
    }
...
# ─────────────────────────────────────────────
# GROQ API CALL (already defined above)
# ─────────────────────────────────────────────
# call_groq already defined at top

# ─────────────────────────────────────────────
# COLUMN DESCRIPTION (for AI context)
# ─────────────────────────────────────────────
def describe_column(df: pd.DataFrame, col: str) -> str:
    ...
    if pd.api.types.is_numeric_dtype(series):
        desc += (f"numeric, {len(series):,} non-null values, "
                 ...
                 f"25th percentile={series.quantile(0.25):.2f}, "
                 f"75th percentile={series.quantile(0.75):.2f}.")
    elif pd.api.types.is_datetime64_any_dtype(series):
        desc += (f"datetime, from {series.min().strftime('%Y-%m-%d')} "
                 f"to {series.max().strftime('%Y-%m-%d')}, "
                 f"spanning {(series.max() - series.min()).days} days, "
                 f"missing {n_missing} ({pct_missing:.1f}%).")
    else:
        top5 = series.value_counts().head(5)
        top_str = ", ".join([f"'{k}' ({v} rows)" for k, v in top5.items()])
        desc += (f"categorical, {series.nunique():,} unique values, "
                 f"missing {n_missing} ({pct_missing:.1f}%), "
                 f"top categories: {top_str}.")
    return desc

# ─────────────────────────────────────────────
# AI CHART SUGGESTION (uses Groq)
# ─────────────────────────────────────────────
def ai_suggest_charts(df: pd.DataFrame, schema: dict, primary: str | None) -> list[dict] | None:
    col_descriptions = []
    for col in df.columns[:20]:
        col_descriptions.append(describe_column(df, col))
    if len(df.columns) > 20:
        col_descriptions.append(f"... and {len(df.columns)-20} more columns.")
    sample = df.head(3).to_dict(orient="records")
    sample_str = json.dumps(sample, indent=2, default=str)[:500]
    prompt = f"""
You are a data visualization expert. Analyze the dataset described below and suggest the most appropriate charts to display insights.

Column descriptions:
{chr(10).join(col_descriptions)}

Primary numeric measure (if any): {primary or "None"}

Small sample (first 3 rows):
{sample_str}

Return a JSON list of chart recommendations. Each recommendation must have:
- "x": column name for X axis (or null)
- "y": column name for Y axis (or null)
- "color": column for color grouping (or null)
- "title": a human‑readable title (in English or Arabic, based on data language)
- "type": one of ["line", "bar", "pie", "scatter", "histogram", "box", "heatmap", "treemap", "funnel", "violin", "cumulative", "rolling_avg", "pareto"]
- "description": a short explanation of what this chart shows.

Focus on meaningful relationships. Prefer date columns for X in time series. Prefer categorical columns for color. Suggest at most 6 charts.
Return ONLY the JSON array, no other text.
"""
    try:
        response = call_groq([{"role": "user", "content": prompt}])
        ...
        st.warning(f"AI chart suggestion failed: {e}. Using fallback auto selection.")
        return None
...
def build_dataset_context(df: pd.DataFrame, schema: dict, primary: str | None, health: dict) -> str:
    ...
    for i, col in enumerate(df.columns):
        if i >= 15:
            ...
            break
        lines.append(describe_column(df, col))
    if primary:
        lines.append(f"Primary measure: '{primary}'.")
        ...
        if schema["date"]:
            ...
            try:
                ...
                if len(monthly) >= 2:
                    ...
                    lines.append(f"Best month: {monthly.idxmax().strftime('%B %Y')} with {fmt_number(monthly.max())}.")
            except Exception:
                pass
        num_cols = schema["numeric"]
        if len(num_cols) >= 2 and primary in num_cols:
            corr = df[num_cols].corr()[primary].drop(primary)
            if not corr.empty:
                best = corr.abs().idxmax()
                lines.append(f"'{primary}' is most correlated with '{best}' (r={corr[best]:.3f}).")
    return " ".join(lines)
...
def render_ai_analyst_tab(df: pd.DataFrame, schema: dict, primary: str | None, health: dict):
    st.markdown("""
    <style>
    .ai-chat-wrap { display: flex; flex-direction: column; gap: 14px; max-height: 540px; overflow-y: auto; padding: 1.2rem 1rem; background: rgba(13,15,20,0.6); border: 1px solid rgba(255,255,255,0.06); border-radius: 16px; scroll-behavior: smooth; }
    .chat-msg-user { align-self: flex-end; background: rgba(79,142,247,0.18); border: 1px solid rgba(79,142,247,0.3); border-radius: 14px 14px 4px 14px; padding: .75rem 1.1rem; max-width: 76%; font-size: .9rem; color: #e8edf5; line-height: 1.55; }
    .chat-msg-ai { align-self: flex-start; background: rgba(24,29,42,0.9); border: 1px solid rgba(255,255,255,0.09); border-radius: 4px 14px 14px 14px; padding: .85rem 1.2rem; max-width: 88%; font-size: .9rem; color: #c4cdd8; line-height: 1.65; position: relative; }
    .chat-msg-ai::before { content: '◈'; position: absolute; top: -10px; left: -4px; font-size: .8rem; color: #4f8ef7; background: #080a0f; padding: 0 4px; border-radius: 4px; }
    .chat-timestamp { font-size: .65rem; color: #556070; margin-top: 4px; text-align: right; }
    </style>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""<script>var chatBox = document.getElementById('ai-chat-box'); if (chatBox) chatBox.scrollTop = chatBox.scrollHeight;</script>""", unsafe_allow_html=True)
...
def _expiry_date(plan: str) -> str:
    days = 30 if plan == "free" else 36500
...
def is_pro() -> bool:
    return st.session_state.get("plan", "free") == "pro"
...
def render_plan_toggle():
    ...
    if st.sidebar.button("🔄 Toggle Plan (Demo)", key="toggle_plan_btn", use_container_width=True):
        ...
        st.rerun()
    st.sidebar.caption(f"Current: {'Pro' if plan=='pro' else 'Free'} · Session `{st.session_state.get('session_id', '—')}`")

def render_page_nav():
    st.sidebar.markdown("---")
    st.sidebar.markdown("<div class='sidebar-section'>Navigation</div>", unsafe_allow_html=True)
    pages = [("dashboard", "📊 Dashboard"), ("my_dashboards", "📂 My Dashboards"), ("history", "🕑 History")]
...
def render_my_dashboards_page():
    st.markdown("<style>.plan-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 12px;border-radius:99px;font-size:.68rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;}.plan-free{background:rgba(100,116,139,0.15);border:1px solid rgba(100,116,139,0.3);color:#94a3b8;}.plan-pro{background:rgba(167,139,250,0.15);border:1px solid rgba(167,139,250,0.4);color:#a78bfa;}.db-card{background:linear-gradient(135deg,rgba(24,29,42,0.95) 0%,rgba(19,22,31,0.95) 100%);border:1px solid rgba(255,255,255,0.085);border-radius:16px;padding:1.4rem 1.6rem;position:relative;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.4);transition:border-color .25s,transform .25s,box-shadow .25s;margin-bottom:14px;}.db-card:hover{border-color:rgba(79,142,247,0.35);transform:translateY(-2px);box-shadow:0 12px 40px rgba(0,0,0,0.5),0 0 30px rgba(79,142,247,0.1);}.db-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,#4f8ef7,#a78bfa,transparent);}.db-card-title{font-size:1.05rem;font-weight:700;color:#e8edf5;margin-bottom:.3rem;letter-spacing:-.01em;}.db-card-sub{font-size:.8rem;color:#556070;margin-bottom:.85rem;}.db-card-meta{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:.85rem;}.db-meta-chip{font-size:.68rem;font-weight:600;padding:2px 9px;border-radius:99px;background:rgba(79,142,247,0.08);border:1px solid rgba(79,142,247,0.15);color:#7fb3f7;letter-spacing:.02em;}.status-badge{display:inline-flex;align-items:center;gap:5px;font-size:.68rem;font-weight:700;padding:3px 10px;border-radius:99px;letter-spacing:.04em;}.status-active{background:rgba(52,211,153,0.12);border:1px solid rgba(52,211,153,0.3);color:#34d399;}.status-soon{background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.3);color:#f59e0b;}.status-expired{background:rgba(248,113,113,0.12);border:1px solid rgba(248,113,113,0.3);color:#f87171;}.upgrade-banner{background:linear-gradient(135deg,rgba(167,139,250,0.12) 0%,rgba(79,142,247,0.08) 100%);border:1px solid rgba(167,139,250,0.25);border-radius:14px;padding:1.1rem 1.4rem;display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:1.5rem;}.upgrade-banner-text{font-size:.88rem;color:#c4cdd8;}.upgrade-banner-text b{color:#a78bfa;}</style>", unsafe_allow_html=True)
    plan = st.session_state.get("plan", "free")
    ...
    if plan == "free":
        ...
        <div class='upgrade-banner'>
            <div class='upgrade-banner-text'>⚡ <b>Free Plan</b>: 1 dashboard · 100-row preview · 30-day expiry · No AI chat · No export &nbsp;→ Upgrade to <b>Pro</b> for unlimited everything.</div>
    ...
    for dash_id, rec in list(dashboards.items()):
        ...
        <div class='db-card'>
            <div style='display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:.5rem'>
                <div><div class='db-card-title'>◈ {rec["name"]}</div><div class='db-card-sub'>Dataset: {rec["dataset"]}</div></div>
                <div style='display:flex;flex-direction:column;align-items:flex-end;gap:5px'>{status}<span class='plan-badge {"plan-pro" if rec.get("plan")=="pro" else "plan-free"}'>{"✦ Pro" if rec.get("plan")=="pro" else "⚡ Free"}</span></div>
            </div>
            <div class='db-card-meta'>
                ...
                <span class='db-meta-chip' style='color:{countdown_color};border-color:{countdown_color}44'>⏳ {days}d remaining</span>
            </div>
            <div style='font-size:.75rem;color:#556070;font-family:"DM Mono",monospace;margin-bottom:.5rem;word-break:break-all'>{rec.get("url","—")}</div>
        </div>
        """, unsafe_allow_html=True)
...
def render_history_page():
    st.markdown("""
    <style>
    .hist-card{background:rgba(19,22,31,0.9);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:1.1rem 1.4rem;margin-bottom:10px;display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr auto;align-items:center;gap:12px;transition:border-color .2s,box-shadow .2s;}
    .hist-card:hover{border-color:rgba(79,142,247,0.25);box-shadow:0 4px 20px rgba(0,0,0,0.3);}
    .hist-card-name{font-size:.9rem;font-weight:700;color:#e8edf5;}
    .hist-card-sub{font-size:.72rem;color:#556070;margin-top:2px;}
    .hist-col-label{font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.09em;color:#556070;}
    .hist-col-val{font-size:.85rem;font-weight:600;color:#c4cdd8;font-family:'DM Mono',monospace;}
    .countdown-card{background:linear-gradient(135deg,rgba(24,29,42,0.95) 0%,rgba(19,22,31,0.95) 100%);border:1px solid rgba(245,158,11,0.2);border-radius:12px;padding:.9rem 1.2rem;display:flex;align-items:center;gap:12px;}
    .countdown-days{font-size:2rem;font-weight:800;font-family:'DM Mono',monospace;color:#f59e0b;line-height:1;}
    </style>
    """, unsafe_allow_html=True)
    dashboards = st.session_state.get("dashboards", {})
    plan = st.session_state.get("plan", "free")
    ...
    if not dashboards:
        ...
        return
    st.markdown("""
    <div style='display:grid;grid-template-columns:2fr 1.2fr 1.2fr 0.8fr 0.8fr 1.1fr auto;gap:12px;padding:.5rem 1.4rem;margin-bottom:6px'>
        <div class='hist-col-label'>Dashboard Name</div><div class='hist-col-label'>Created</div><div class='hist-col-label'>Last Opened</div>
        <div class='hist-col-label'>Rows</div><div class='hist-col-label'>Cols</div><div class='hist-col-label'>Status</div><div class='hist-col-label'>ID</div>
    </div>
    """, unsafe_allow_html=True)
    for dash_id, rec in list(dashboards.items()):
        days = _days_remaining(rec.get("expiry", "2099-01-01"))
        ...
        st.markdown(f"""
        <div class='hist-card'>
            <div><div class='hist-card-name'>◈ {rec["name"]}</div><div class='hist-card-sub'>{rec.get("dataset","—")}</div></div>
        ...
        for i, (dash_id, rec) in enumerate(list(dashboards.items())[:4]):
            ...
            with cols_exp[i % len(cols_exp)]:
                ...
                <div class='countdown-card'>
                    ...
                    <div><div style='font-size:.75rem;color:#e8edf5;font-weight:700'>{rec["name"][:20]}</div><div style='font-size:.68rem;color:#556070'>days remaining</div><div style='font-size:.65rem;color:#556070;margin-top:2px'>Expires {rec.get("expiry","—")}</div></div>
                </div>
                """, unsafe_allow_html=True)
...
def main_enhanced():
    ...
    dataset_type = "Time Series" if schema["date"] else "Cross-sectional" if schema["categorical"] else "Numeric"
    ...
    with tab_dashboard:
        ...
        if "ai_charts" not in st.session_state:
            with st.spinner("🤖 AI is analyzing your data to suggest the best charts..."):
                suggested = ai_suggest_charts(df, schema, primary)
                if suggested:
                    st.session_state.ai_charts = suggested
                else:
                    st.session_state.ai_charts = None

        if st.session_state.ai_charts:
            for chart_spec in st.session_state.ai_charts:
                ...
                try:
                    if chart_type == "line" and x_col and y_col:
                        fig = chart_time_series(df, x_col, y_col)
                    elif chart_type == "bar" and x_col and y_col:
                        fig = chart_category_bar(df, x_col, y_col)
                    elif chart_type == "pie" and x_col and y_col:
                        fig = chart_category_pie(df, x_col, y_col)
                    elif chart_type == "scatter" and x_col and y_col:
                        fig = chart_scatter(df, x_col, y_col, color_col)
                    elif chart_type == "histogram" and y_col:
                        fig = chart_distribution(df, y_col)
                    elif chart_type == "box" and x_col and y_col:
                        fig = chart_box(df, x_col, y_col)
                    elif chart_type == "heatmap" and x_col and y_col:
                        fig = chart_monthly_heatmap(df, x_col, y_col)
                    elif chart_type == "treemap" and x_col and y_col:
                        fig = chart_treemap(df, x_col, y_col)
                    elif chart_type == "funnel" and x_col and y_col:
                        fig = chart_funnel(df, x_col, y_col)
                    elif chart_type == "violin" and x_col and y_col:
                        fig = chart_violin(df, x_col, y_col)
                    elif chart_type == "cumulative" and x_col and y_col:
                        fig = chart_cumulative(df, x_col, y_col)
                    elif chart_type == "rolling_avg" and x_col and y_col:
                        fig = chart_rolling_avg(df, x_col, y_col)
                    elif chart_type == "pareto" and x_col and y_col:
                        fig = chart_pareto(df, x_col, y_col)
                    if fig:
            ...
            # Fallback to original auto selection
